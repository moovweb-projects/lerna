...
function userGetVersion(currentVersion, pkgName, masterVersion) {
  var message = "New version ";
  if (pkgName) message += "for \"" + pkgName + "\" ";
  message += "(Leave blank for patch version): ";

  var input = masterVersion || readline.question(message);
...
module.exports = function publishChangedPackages(
  ...
  try {
    ...
    if (flags.canary) {
      ...
      masterVersion = userGetVersion(currentVersion, undefined, flags.master);
